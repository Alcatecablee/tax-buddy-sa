...
class AIValidationService {
  private readonly API_BASE = "/api/ai-validation";
  private readonly fallbackValidation = true; // Use intelligent fallback when AI service unavailable
  ...
  async validateField(
    ...
    // Input validation
    if (field == null || field === "") {
      throw new Error("Field name is required for validation");
    }

    try {
      // First, try the real AI service
      const response = await this.callAIService(field, value, context);
      if (response) {
        return response;
      }
    } catch (error) {
      console.warn(
        "AI service unavailable, using intelligent fallback validation:",
        error,
      );
    }

    // Intelligent fallback validation
    return this.performIntelligentValidation(field, value, context);
  }
  ...
  private performIntelligentValidation(
    field: string,
    value: any,
    context: ValidationContext,
  ): ValidationResult {
  ...
  private validateRetirementContribution(
    value: number,
    context: ValidationContext,
  ): ValidationResult {
    const suggestions: string[] = [];
  ...
  private validateMedicalContribution(
    value: number,
    context: ValidationContext,
  ): ValidationResult {
    const suggestions: string[] = [];
  ...
  private validateMedicalCredits(
    value: number,
    context: ValidationContext,
  ): ValidationResult {
    if (value < 0) {
  ...
  private validateUIFContribution(
    value: number,
    context: ValidationContext,
  ): ValidationResult {
    if (value < 0) {
  ...
  private validateTravelAllowance(
    value: number,
    context: ValidationContext,
  ): ValidationResult {
    if (value < 0) {
  ...
  private calculateExpectedPAYE(annualSalary: number): number {
    // Simplified PAYE calculation for 2024/2025
    // This is a basic calculation - real implementation would be more complex
    const taxBrackets = [
